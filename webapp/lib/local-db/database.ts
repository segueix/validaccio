...
function upgradeDatabase(
  ...
  const links = database.objectStoreNames.contains(
    LOCAL_DATABASE_SCHEMA.stores.links,
  )
    ? transaction.objectStore(LOCAL_DATABASE_SCHEMA.stores.links)
    : database.createObjectStore(LOCAL_DATABASE_SCHEMA.stores.links, {
        keyPath: "id",
      });

  if (!links.indexNames.contains("projectId")) {
    links.createIndex("projectId", "projectId");
  }
  if (!links.indexNames.contains("affirmationId")) {
    links.createIndex("affirmationId", "affirmationId");
  }
  if (!links.indexNames.contains("evidenceId")) {
    links.createIndex("evidenceId", "evidenceId");
  }

  const cells = database.objectStoreNames.contains(
    LOCAL_DATABASE_SCHEMA.stores.cells,
  )
    ? transaction.objectStore(LOCAL_DATABASE_SCHEMA.stores.cells)
    : database.createObjectStore(LOCAL_DATABASE_SCHEMA.stores.cells, {
        keyPath: "id",
      });

  if (!cells.indexNames.contains("projectId")) {
    cells.createIndex("projectId", "projectId");
  }
  if (!cells.indexNames.contains("evidenceId")) {
    cells.createIndex("evidenceId", "evidenceId");
  }
  ...
  const chapterDrafts = database.objectStoreNames.contains(
    LOCAL_DATABASE_SCHEMA.stores.chapterDrafts,
  )
    ? transaction.objectStore(LOCAL_DATABASE_SCHEMA.stores.chapterDrafts)
    : database.createObjectStore(LOCAL_DATABASE_SCHEMA.stores.chapterDrafts, {
        keyPath: "id",
      });
  if (!chapterDrafts.indexNames.contains("projectId")) {
    chapterDrafts.createIndex("projectId", "projectId");
  }
  if (!chapterDrafts.indexNames.contains("manuscriptId")) {
    chapterDrafts.createIndex("manuscriptId", "manuscriptId");
  }

  const chapterVersions = database.objectStoreNames.contains(
    LOCAL_DATABASE_SCHEMA.stores.chapterVersions,
  )
    ? transaction.objectStore(LOCAL_DATABASE_SCHEMA.stores.chapterVersions)
    : database.createObjectStore(
        LOCAL_DATABASE_SCHEMA.stores.chapterVersions,
        { keyPath: "id" },
      );
  if (!chapterVersions.indexNames.contains("projectId")) {
    chapterVersions.createIndex("projectId", "projectId");
  }
  if (!chapterVersions.indexNames.contains("manuscriptId")) {
    chapterVersions.createIndex("manuscriptId", "manuscriptId");
  }
